...
def register(bot):
    ...
    async def approve(interaction: Interaction):
        ...
        if not sess:
            await interaction.followup.send("No draft to approve. Use `/propose_tweet` or `/reply_to_tweet` first.", ephemeral=True)
            return
        ...
        if not tokens:
            ...
            return
        try:
            result = post_tweet(tokens['access_token'], sess['draft'], in_reply_to=sess.get('in_reply_to'))
            url = f"https://x.com/{tokens.get('screen_name')}/status/{result['data']['id']}"
            await interaction.followup.send(("Reply posted: " if sess.get('in_reply_to') else "Posted: ") + url, ephemeral=True)
